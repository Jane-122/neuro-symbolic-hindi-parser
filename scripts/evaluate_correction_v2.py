...
def main():
    args = parse_args()
    split = args.split
    baseline_path = Path(f"results/stanza_{split}_baseline_all.csv")
    corrected_path = Path(f"results/stanza_{split}_corrected_v2_all.csv")
    output_path = Path(f"output/{split}_correction_v2_metrics.csv")
