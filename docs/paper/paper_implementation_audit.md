...
- Repository outputs under `outputs/` and `experiments/stanza/` (and subfolders)
...
- Gold Karaka mapping (`k1`…`k7p`) matches `scripts/data_prep/extract_gold_karaka_labels.py`.
...
- Test per-Karaka strict F1 table (Table V) matches `outputs/metrics/test_correction_v2_metrics.csv`.
...
4. **Section III.C / Table III:** Rules are described as "designed by analysing … UD Hindi-HDTB" plus Paninian analysis. This is mostly true, but should not be read as "taken directly from literature." Primary evidence is **train/dev postposition statistics** (`notebooks/exploration/02_postposition_analysis.ipynb`).
...
- **Train token matching counts** were not recorded in `outputs/metrics/train_correction_v2_metrics.csv` (older CSV schema without `matched_gold_rows`). Train denominator is verified as **47378**, but train matched/unmatched breakdown was not exported.
...
| 13302 | Aligned train sentences | `outputs/alignment/aligned_ud_hdtb_sentences.csv` (`ud_split=train`) | **Verified** |
| 1659 | Aligned dev sentences | same file (`ud_split=dev`) | **Verified** |
| 1684 | Aligned test sentences | same file (`ud_split=test`) | **Verified** |
| 47378 | Gold Karaka train rows | `outputs/gold/gold_karaka_labels.csv` | **Verified** |
| 5902 | Gold Karaka dev rows | same | **Verified** |
| 5946 | Gold Karaka test rows | same | **Verified** |
| 5924 | Test matched gold rows | `outputs/metrics/test_correction_v2_metrics.csv` (`matched_gold_rows`) | **Verified** |
...
| 1553 | Dev neural-only errors | `outputs/error_analysis/dev_neural_only_errors.csv` (row count) | **Verified** |
| 524 | Dev actionable errors | `outputs/error_analysis/dev_actionable_neural_errors.csv` (row count) | **Verified** |
| 80 | Adhikarana \| nmod \| में (dev) | `outputs/error_analysis/dev_actionable_neural_errors.csv` (pattern count) | **Verified** |
...
| 95.16% | DR1 original deprel accuracy | `outputs/rejected_experiments/dependency_repair_v1/dependency_repair_v1_dev_eval.csv` | **Verified** |
| 94.85% | DR1 repaired deprel accuracy | same | **Verified** |
| 121 / 5 / 113 | DR1 repair outcomes | same | **Verified** |
| 0.7356 / 0.5621 / 0.4850 / 0.3876 | Test neural-only metrics | `outputs/metrics/test_correction_v2_metrics.csv` | **Verified** |
...
| 0.7865 / 0.6023 / 0.5310 / 0.4234 | Train neural-only | `outputs/metrics/train_correction_v2_metrics.csv` | **Verified** |
| 0.7864 / 0.6023 / 0.6259 / 0.4451 | Train verifier | same | **Verified** |
| 0.8004 / 0.6073 / 0.6400 / 0.4513 | Train correction | same | **Verified** |
| 0.7369 / 0.5599 / 0.4909 / 0.3877 | Dev neural-only | `outputs/metrics/dev_correction_v2_metrics.csv` | **Verified** |
...
| +0.0424 | Adhikarana candidate F1 (verifier→corr, test) | `outputs/metrics/test_correction_v2_per_karaka_comparison.csv` | **Verified** |
...
All train and dev overall metrics in Section 2 of this audit match `outputs/metrics/train_correction_v2_metrics.csv` and `outputs/metrics/dev_correction_v2_metrics.csv` exactly (4 decimal places).
...
All six Karakas match `outputs/metrics/test_correction_v2_metrics.csv` strict per_karaka rows exactly.
...
2. **Stale output risk:** `outputs/metrics/train_correction_v2_metrics.csv` uses system name `correction_v2` and omits match-count columns, suggesting an older evaluator variant. Numbers match final documentation, but schema differs from dev/test files.
...
4. **Verifier rule firing counts:** No consolidated CSV of R1–R5 fire counts on test was found in `outputs/`. Paper does not claim these counts; add only if computed from baseline CSVs.
...
| Task | Script / file |
|------|---------------|
| Sentence alignment | `scripts/data_prep/build_aligned_hdtb_ud.py` → `outputs/alignment/aligned_ud_hdtb_sentences.csv` |
| Gold Karaka extraction | `scripts/data_prep/extract_gold_karaka_labels.py` → `outputs/gold/gold_karaka_labels.csv` |
| Stanza baseline | `src/pipeline/run_stanza_baseline.py --split {train,dev,test}` |
| Apply H1 correction | `scripts/evaluation/apply_correction_v2.py --split {split}` |
| Final metrics | `scripts/evaluation/evaluate_correction_v2.py --split {split}` |
| Per-Karaka comparison | `scripts/evaluation/compare_correction_v2_per_karaka.py --split {split}` |
| Dev error mining | `scripts/analysis/analyze_dev_neural_errors.py`, `scripts/analysis/analyze_dev_actionable_errors.py` |
| DR1 evaluation | `scripts/experiments/evaluate_dependency_repair_v1_dev.py` → `outputs/rejected_experiments/dependency_repair_v1/dependency_repair_v1_dev_eval.csv` |
