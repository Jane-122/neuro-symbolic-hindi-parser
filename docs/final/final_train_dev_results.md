...
Source: `outputs/metrics/train_correction_v2_metrics.csv`
...
Source: `outputs/metrics/dev_correction_v2_metrics.csv`
...
Source: `outputs/metrics/dev_correction_v2_per_karaka_comparison.csv`
...
## 7. Post-Freeze Note: UDPipe Dev Robustness Evaluation

After the Stanza system freeze, a **UDPipe parser branch** was added for cross-parser robustness validation. The same frozen Mapper v1, Verifier v1, and Correction Layer v2.1 (H1 only) were applied without rule changes.

**UDPipe dev results (Correction v2.1):**

| Metric | Stanza dev | UDPipe dev |
|--------|----------:|----------:|
| Candidate accuracy | 0.7574 | 0.7135 |
| Strict accuracy | 0.6118 | 0.5701 |
| Adhikarana strict F1 | 0.8335 | 0.8322 |
| Token match rate | 99.63% | 99.63% |
| H1 firing count | 121 | 132 |

Source: `outputs/metrics/dev_correction_v2_metrics.csv`, `outputs/metrics/udpipe_dev_correction_v2_metrics.csv`.

UDPipe dev strict accuracy is lower than Stanza, but Adhikarana gains from H1 and token matching are nearly identical — supporting parser-robustness claims without retuning the symbolic layer.

Held-out **Stanza test** results are documented in `docs/final/final_test_results.md`. Cross-parser **test** comparison is in Section 8 of that document.

## 8. Reporting Note (Historical)
