...
Source: `outputs/metrics/test_correction_v2_metrics.csv`, `outputs/metrics/test_correction_v2_per_karaka_comparison.csv`
...
## 8. Cross-Parser Robustness Evaluation

After the Stanza test freeze, the **same frozen symbolic stack** was applied to **UDPipe** Hindi-HDTB parses for robustness validation only. No mapper, verifier, correction, or evaluation formula changes were made.

Source: `outputs/metrics/udpipe_test_correction_v2_metrics.csv`, `outputs/metrics/udpipe_dev_correction_v2_metrics.csv`, token matching audits.

### Correction v2.1 — Test Comparison

| Metric | Stanza | UDPipe |
|--------|-------:|-------:|
| Token matching (matched / total) | 5,924 / 5,946 | 5,924 / 5,946 |
...
- **Token alignment** is parser-independent (same 22 unmatched rows per split).
- **Adhikarana / H1 behavior** transfers across parsers with negligible F1 difference on test.
- **Overall strict accuracy** is lower for UDPipe, consistent with parser-quality differences rather than symbolic rule failure.
- This experiment validates **robustness**, not a second tuned system.

See also: `docs/final/project_update_after_freeze.md`, `outputs/error_analysis/paper_error_analysis_table_v2.md`.
